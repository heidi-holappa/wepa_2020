...
//    Message findByUser(Account user);
    ...
    @Query(value = "SELECT DISTINCT(M.id), M.comments, M.content, M.likes, M.message_date, M.user_id, M.op_id FROM  MESSAGE M LEFT JOIN USER_INFO_FRIENDS UIF ON M.USER_ID = UIF.FRIENDS_ID LEFT JOIN USER_INFO UI ON UIF.USER_INFO_ID = UI.ID WHERE (UI.USER_ID = 2 AND M.OP_ID = 0) OR (M.USER_ID = 2 AND M.OP_ID = 0) ORDER BY MESSAGE_DATE DESC LIMIT 25", nativeQuery = true)
